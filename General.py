...
randomize_aliases = [
    ...
    "shuffle",
    "choose",
]  # edit randomize() if u change this, shuffle on the left, choose on the right
feedback_aliases = [
    "request",
    "issue",
    "bug",
    "report",
]  # make sure u change feedback_type in feedback() if you edit this, bugs on the right, requests on the left
...
async def say(client, message, args, is_edit=False):
    ...
    # get the channel
    channel = (
        Support.get_id_from_str(args[1 if is_edit else 0])
        if "#" in args[1 if is_edit else 0]
        else []
    )
    ...
    if args[0].strip():  # content left over
        ...
        content = (
            content[len(channel.mention) :] if args[0] == channel.mention else content
        )
        content = f"```{content}```" if markdown else content
        content = content.replace("\\s", Support.emojis.space_char).replace(
            "\\b", Support.emojis.bullet
        )
        ...
        log("edit", "edit command rejected") if msg else log(
            "say", "say command rejected"
        )
...
async def feedback(client, message, args):
    ...
    if not args[1] or args[1] in Help.help_aliases:  # no feedback provided
        await Help.send_help_embed(
            ...
            else Help.help_links,  # FIXME set these links
            default_footer=False,
        )
        return

    channel = client.get_guild(Support.ids.phyner_support_id).get_channel(
        Support.ids.reported_issues
        if feedback_type == "issue"
        else Support.ids.requested_features
    )
    ...
    for r in [
        Support.emojis.pushpin_emoji,
        Support.emojis.wrench_emoji,
        Support.emojis.tick_emoji,
        Support.emojis.wastebasket_emoji,
    ]:
...
async def reaction(message, args):
    ...
    channel = (
        message.channel_mentions[0] if message.channel_mentions else message.channel
    )

    try:
        msg_id = re.findall(r"\s\d{19}\s", " ".join(args))[0].strip()
